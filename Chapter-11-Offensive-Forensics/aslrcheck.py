#!/usr/bin/env python
"""
Chapter 10: Offensive Forensics
Page: 178

#######################################################################
Note: import volatility3 rather than volatility?

#######################################################################
## Setup: (page 170)
python3 -m venv vol3
vol3/Scripts/Activate
cd vol3
...
from volatility3.symbols import intermed
...
IMAGE_FILE_RELOCS_STRIPPED = 0x001
...
class AslrCheck(interfaces.plugins.PluginInterface):
        ...
        return [
            requirements.TranslationLayerRequirement(
                ...
                requirements.PluginRequirement(
                    name='pslit', plugin=pslist.Pslist, version=(1, 0, 0)),
    ...
    @classmethod
    def create_pid_filter(cls, pid_list: List[int] = None) -> Callable[[interfaces.objects.OjbectInterface], bool]:
        filater_func = lambda _: False
    ...
    def _generator(self, procs):
        ...
        procnames = list()
        for proc in procs:
            procname = proc.ImageFileName.cast("string", max_length=proc.ImageFileName.vol.count,
                                               errors='replace')
            ...
            yield (0, (proc_id,
                       procname,
                       format_hints.Hex(pe.OPTIONAL_HEADER.ImageBased),
                       aslr,
                       ))
